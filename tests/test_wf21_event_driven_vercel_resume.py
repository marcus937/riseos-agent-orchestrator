...
class FakeHermesClient:
    ...
    async def post_runtime_validation(self, base_url: str, token: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"status": "PASSED", "job_id": f"job-{len(self.posts)}"}
...
def test_workflow_waits_for_deployment_without_launching_hermes() -> None:
...
def test_ready_deployment_status_resumes_waiting_workflow_with_preview_url(monkeypatch: Any) -> None:
    ...
    waiting_request = run(runtime_validation_request_from_parsed(parse_github_event("pull_request", pull_request_payload()), settings(), github_client=github))
    waiting = run(store.trigger(waiting_request, settings()))
    ready_request = run(runtime_validation_request_from_parsed(parse_github_event("deployment_status", deployment_status_payload(target_url)), settings(), github_client=github))
    resumed = run(store.trigger(ready_request, settings()))

    assert waiting.status == "pending"
    assert ready_request.target_url == target_url
    assert resumed.hermes.job_id == "job-1"
    assert hermes.posts[0]["targetUrl"] == target_url
...
def test_duplicate_ready_deployment_status_does_not_launch_hermes_twice(monkeypatch: Any) -> None:
    ...
    github.pulls = [pull_request_record()]
    target_url = "https://jarvis-mission-control-git-codex-m2-happy-path-marcus937.vercel.app"
    ready = parse_github_event("deployment_status", deployment_status_payload(target_url))
    request = run(runtime_validation_request_from_parsed(ready, settings(), github_client=github))
...
def test_deployment_status_pending_does_not_start_hermes() -> None:
    github = FakeGitHubClient()
    github.pulls = [pull_request_record()]
    target_url = "https://jarvis-mission-control-git-codex-m2-happy-path-marcus937.vercel.app"
    parsed = parse_github_event("deployment_status", deployment_status_payload(target_url, state="pending"))
